...
const listarFichas = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ erro: "Erro interno do servidor ao buscar fichas." });
  }
};
...
const buscarFichaPorId = async (req, res) => {
  try {
    ...
    // Busca a ficha principal, já trazendo proficiências direto da tabela fichas
    ...
    if (fichaError) {
      if (fichaError.code === "PGRST116") {
        return res.status(404).json({
          erro: "Ficha não encontrada ou você não tem permissão para vê-la.",
        });
      }
      throw fichaError;
    }

    // Busca os poderes relacionados
    ...
    const fichaCompleta = {
      ...ficha,
      poderes: poderes.map((p) => p.poderes),
      magias: magias.map((m) => m.magias),
      proficiencias: ficha.proficiencias ? JSON.parse(ficha.proficiencias) : [],
    };
...
const LISTA_PERICIAS_COMPLETA = {
  Acrobacia: { atributo_base: "destreza" },
  Adestramento: { atributo_base: "carisma" },
  Atletismo: { atributo_base: "forca" },
  Atuação: { atributo_base: "carisma" },
  Cavalgar: { atributo_base: "destreza" },
  Conhecimento: { atributo_base: "inteligencia" },
  Cura: { atributo_base: "sabedoria" },
  Diplomacia: { atributo_base: "carisma" },
  Enganação: { atributo_base: "carisma" },
  Fortitude: { atributo_base: "constituicao" },
  Furtividade: { atributo_base: "destreza" },
  Guerra: { atributo_base: "inteligencia" },
  Iniciativa: { atributo_base: "destreza" },
  Intimidação: { atributo_base: "carisma" },
  Intuição: { atributo_base: "sabedoria" },
  Investigação: { atributo_base: "inteligencia" },
  Jogatina: { atributo_base: "carisma" },
  Ladinagem: { atributo_base: "destreza" },
  Luta: { atributo_base: "forca" },
  Misticismo: { atributo_base: "inteligencia" },
  Nobreza: { atributo_base: "inteligencia" },
  Ofício: { atributo_base: "inteligencia" },
  Percepção: { atributo_base: "sabedoria" },
  Pilotagem: { atributo_base: "destreza" },
  Pontaria: { atributo_base: "destreza" },
  Reflexos: { atributo_base: "destreza" },
  Religião: { atributo_base: "sabedoria" },
  Sobrevivência: { atributo_base: "sabedoria" },
...
const criarFicha = async (req, res) => {
  ...
  try {
    ...
    if (poderes && poderes.length > 0) {
      ...
      const { error: poderesError } = await supabase
        .from("fichas_poderes")
        .insert(poderesParaInserir);
      if (poderesError) throw poderesError;
    }
    ...
    if (magias && magias.length > 0) {
      ...
      const { error: magiasError } = await supabase
        .from("fichas_magias")
        .insert(magiasParaInserir);
      if (magiasError) throw magiasError;
    }
    ...
    res.status(500).json({
      erro: "Ocorreu um erro no servidor ao tentar criar a ficha completa.",
    });
  }
};
...
const deletarFicha = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ erro: "Erro interno do servidor ao deletar a ficha." });
  }
};
...
const atualizarInfoGeral = async (req, res) => {
  ...
  if (!identidade || !identidade.nomePersonagem) {
    return res
      .status(400)
      .json({ erro: "O nome do personagem é obrigatório." });
  }
...
const atualizarPericias = async (req, res) => {
  ...
  if (!pericias)
    return res.status(400).json({ erro: "Dados de perícias não fornecidos." });
  ...
  try {
    ...
    res
      .status(500)
      .json({ erro: "Erro interno do servidor ao atualizar as perícias." });
  }
};
...
const atualizarPontos = async (req, res) => {
  ...
  try {
    const { data: ficha, error: fetchError } = await supabase
      .from("fichas")
      .select(
        "pontos_de_vida_atual, pontos_de_vida_max, pontos_de_mana_atual, pontos_de_mana_max"
      )
      .match({ id: id, user_id: req.userId })
      .single();
    ...
    let updateData = {};
    if (tipo === "pv") {
      updateData.pontos_de_vida_atual = Math.max(
        0,
        Math.min(
          ficha.pontos_de_vida_atual + valor,
          pvMaximo || ficha.pontos_de_vida_max
        )
      );
      if (pvMaximo) updateData.pontos_de_vida_max = pvMaximo;
    } else if (tipo === "pm") {
      updateData.pontos_de_mana_atual = Math.max(
        0,
        Math.min(
          ficha.pontos_de_mana_atual + valor,
          pmMaximo || ficha.pontos_de_mana_max
        )
      );
    ...
    const { data: updatedFicha, error: updateError } = await supabase
      .from("fichas")
      .update(updateData)
      .eq("id", id)
      .select()
...
const atualizarFoto = async (req, res) => {
  ...
  try {
    if (!req.file)
      return res
        .status(400)
        .json({ erro: "Nenhum arquivo de imagem enviado." });
    ...
    const { error: dbError } = await supabase
      .from("fichas")
      .update({ foto_personagem: urlData.publicUrl })
      .match({ id: id, user_id: req.userId });
    if (dbError) throw dbError;

    res.json({
      sucesso: "Foto do personagem atualizada com sucesso!",
      caminho: urlData.publicUrl,
    });
  } catch (error) {
    res
      .status(500)
      .json({ erro: "Erro interno do servidor ao atualizar a foto." });
  }
};
...
const adicionarHabilidade = async (req, res) => {
  ...
  if (!habilidade || !habilidade.nome)
    return res
      .status(400)
      .json({ message: "Dados da habilidade incompletos." });

  try {
    const { data: ficha, error: fetchError } = await supabase
      .from("fichas")
      .select("habilidades")
      .match({ id, user_id: req.userId })
      .single();
    if (fetchError || !ficha)
    ...
    const { error: updateError } = await supabase
      .from("fichas")
      .update({ habilidades: habilidadesAtuais })
      .eq("id", id);
    if (updateError) throw updateError;
...
const removerHabilidade = async (req, res) => {
  ...
  if (!habilidadeNome)
    return res
      .status(400)
      .json({ message: "Nome da habilidade não fornecido." });
  ...
  try {
    ...
    const habilidadesAtuais = ficha.habilidades || [];
    const novasHabilidades = habilidadesAtuais.filter(
      (hab) =>
        hab.nome.trim().toLowerCase() !== habilidadeNome.trim().toLowerCase()
    );

    if (habilidadesAtuais.length === novasHabilidades.length)
      return res.status(404).json({ message: "Habilidade não encontrada" });

    const { error: updateError } = await supabase
      .from("fichas")
      .update({ habilidades: novasHabilidades })
      .eq("id", id);
    if (updateError) throw updateError;
...
const atualizarClasses = async (req, res) => {
  ...
  try {
    const { data: ficha, error } = await supabase
      .from("fichas")
      .update({ classes: classes, is_multiclass: isMulticlass })
      .match({ id: id, user_id: req.userId })
      .select()
      .single();
    if (error) throw error;
    ...
    res
      .status(500)
      .json({ erro: "Erro interno do servidor ao atualizar classes" });
  }
};
...
const atualizarDefesa = async (req, res) => {
  ...
  try {
    ...
    if (error) throw error;
    res.json({
      mensagem: "Bônus de defesa atualizados com sucesso!",
      ficha: data[0],
    });
  } catch (err) {
...
const buscarStatsGerais = async (req, res) => {
  const { id } = req.params;

  try {
    // Busca apenas as colunas necessárias da tabela 'fichas'
    const { data, error } = await supabase
      .from("fichas")
      .select("proficiencias, deslocamento, tamanho")
      .eq("id", id)
      .single(); // .single() retorna um único objeto em vez de um array

    if (error) {
      // Se .single() não encontrar nada, ele retorna um erro.
      // Podemos tratar isso como um "não encontrado".
    ...
    console.error("Erro ao buscar stats gerais:", error.message);
    res
      .status(500)
      .json({ message: "Erro interno do servidor ao buscar stats gerais." });
  }
};
